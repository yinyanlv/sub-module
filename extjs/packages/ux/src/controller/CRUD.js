Ext.define('Ext.ux.controller.CRUD', {
	...
	addStoreFilters: function(store) {
		...
		store.proxy.extraFilters = me.searchParams || params;
	},
		...
		if (editMode === me.editMode[0]) {
			form.submit({
				...
				failure: function(that, action) {
					...
					Ext.util.Common.errorHandler(action.response);
				}
			});
	...
	exportRecord: function(that) {
		if (!this.getQuery().isValid()) return;
	...
	getExportParams: function() {
		...
		return {
			filters: me.searchParams || filters,
			sorts: me.getSorts(),
			paging: {
				page: 1,
				size: 100000
	...
	getSorts: function() {
		...
		Ext.each(sorters, function(item) {
			sorts.push({
				...
				asc: item.getDirection() === 'ASC' ? true : false
			});
		});
